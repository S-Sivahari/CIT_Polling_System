...
export default function StudentLogin() {
  ...
  useEffect(() => {
    if (session?.user?.role === 'student' && session.user.isValid) {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result?.error) {
        ...
        console.log('✅ Google sign-in successful for student')
  ...
  useEffect(() => {
    ...
    if (session?.user) {
      if (session.user.isValid === true && session.user.role === 'student') {
        toast.dismiss('signin')
        ...
        if (session.user.role === 'not_found') {
          toast.error('Student account not found. Please contact administration.')
      ...
      <div className="dashboard-bg flex items-center justify-center mobile-p">
        ...
        >
          <div className="card text-center">
            <div className="w-16 h-16 bg-primary-500/20 rounded-full flex items-center justify-center mx-auto mb-4 border border-primary-500/30 animate-pulse">
            ...
            <p className="text-slate-300">
              Authenticating your student account
            </p>
