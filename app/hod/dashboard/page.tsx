                            ...
                            {poll.auto_delete_at && (
                              <div className="text-center">
                                <div className="text-sm font-bold text-red-400 bg-red-500/20 border border-red-500/30 rounded-lg px-3 py-1">
                                  Auto-Delete: {getAutoDeleteHours(poll.auto_delete_at)}
                                </div>
                              </div>
                            )}
  ...
  function EditPollModal({ 
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        // Reset auto-delete timestamp to current time + auto-delete days
        const autoDeleteTime = new Date(now.getTime() + autoDeleteDays * 24 * 60 * 60 * 1000).toISOString()
        ...
        const updateData = {
          ...
          target_gender: targetGender,
          auto_delete_days: autoDeleteDays,
          auto_delete_at: (window as any).__hodAutoDeleteOverride || autoDeleteTime,
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    onChange={(e) => {
                      ...
                      const base = new Date()
                      const overrideTime: string = new Date(base.getTime() + days * 24 * 60 * 60 * 1000).toISOString()
                      ;(window as any).__hodAutoDeleteOverride = overrideTime
                    }}
                    className="w-full px-3 py-2 bg-slate-700 border border-amber-500/30 rounded-lg text-white"
                    defaultValue={(pollCategory === 'Attendance' || pollCategory === 'CodeChef Attendance' || pollCategory === 'Problems Solved') ? '1' : '2'}
                  >
                ...
                <div>
                  ...
                  <div className="px-3 py-2 bg-slate-700 border border-amber-500/30 rounded-lg text-amber-200 text-sm">
                    {poll.auto_delete_at ? new Date(poll.auto_delete_at).toLocaleString() : 'Not set'}
                  </div>
                </div>
              </div>
