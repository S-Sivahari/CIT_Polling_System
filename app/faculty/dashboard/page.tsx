...
function EditPollModal({ 
  ...
  const [autoDeleteDays, setAutoDeleteDays] = useState<number>(
    (poll.auto_delete_days as number) || ((poll.poll_category === 'Attendance' || (poll.poll_category as any) === 'CodeChef Attendance' || poll.poll_category === 'Problems Solved') ? 1 : 2)
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-delete timestamp from state
      const autoDeleteTime = new Date(now.getTime() + autoDeleteDays * 24 * 60 * 60 * 1000).toISOString()
