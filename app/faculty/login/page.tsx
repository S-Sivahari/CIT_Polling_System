...
export default function FacultyLogin() {
  ...
  useEffect(() => {
    if (session?.user?.role === 'faculty' && session.user.isValid) {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result?.error) {
        ...
        console.log('✅ Google sign-in successful for faculty')
  ...
  useEffect(() => {
    ...
    if (session?.user) {
      if (session.user.isValid === true && session.user.role === 'faculty') {
        toast.dismiss('signin')
        ...
        if (session.user.role === 'not_found') {
          toast.error('Faculty account not found. Please contact administration.')
      ...
      <div className="dashboard-bg flex items-center justify-center p-4">
        ...
        >
          <div className="card text-center">
            <div className="w-16 h-16 bg-accent-500/20 rounded-full flex items-center justify-center mx-auto mb-4 border border-accent-500/30 animate-pulse">
            ...
            <p className="text-slate-300">
              Authenticating your faculty account
            </p>
        ...
        <div className="card">
          ...
          <div className="space-y-6">
            ...
            <div className="text-center text-sm text-slate-400">
              ...
              <p className="mt-1">Faculty accounts only</p>
            </div>
          </div>
